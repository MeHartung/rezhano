...
/////////////////////////////////////////////////////////////////////////////
/// 
namespace AccurateCommerce\Search\Sphinx;
...
/// current client-side command implementation versions
   const VER_COMMAND_SEARCH = 0x120;
...
/// known grouping functions
  ...
  protected $_mode;			///< query matching mode (default is SPH_MATCH_EXTENDED2)
  ...
  protected $_query_flags; ///< per-query protectedious flags
  protected $_predictedtime; ///< per-query max_predicted_time
  protected $_outerorderby; ///< outer match sort by
  protected $_outeroffset; ///< outer offset
  protected $_outerlimit; ///< outer limit
  protected $_hasouter;
  protected $_token_filter_library; ///< token_filter plugin library name
  protected $_token_filter_name; ///< token_filter plugin name
  protected $_token_filter_opts; ///< token_filter plugin options
...
// unpack 64-bit signed
  ...
  private function sphSetBit ( $flag, $bit, $on )
  {
    if ( $on )
    {
      $flag |= ( 1<<$bit );
  ...
  public function __construct ($options=array())
  {
    $defaultOptions = array(
      ...
      'overrides' => array(),
      'select' => "*",
      'query_flags' => $this->sphSetBit(0, 6, true), // default idf=>tfidf_normalized
      'predictedtime' => 0,
      'outerorderby' => "",
      'outeroffset' => 0,
      'outerlimit' => 0,
      'hasouter' => false,
      'token_filter_library' => '',
      'token_filter_name' => '',
      'token_filter_opts' => '',
      'timeout' => 0
    );
    ...
    foreach ($knownOptions as $key)
    {
      ...
      $this->$prop = isset($options[$key]) ? $options[$key] : null;
    }
  ...
  public function _GetResponse ( $fp, $client_ver )
  {
    ...
    if ( !$response || $read!=$len )
    {
      ...
      throw new SphinxSearchException($this->error);
      return false;
    }
  ...
  public function SetFilterString ( $attribute, $value, $exclude=false )
  {
    ...
    $this->_filters[] = array ( "type"=>self::SPH_FILTER_STRING, "attr"=>$attribute, "exclude"=>$exclude, "value"=>$value );
  }
  ...
  public function SetFilterStringList ( $attribute, $value, $exclude=false )
  {
    ...
    $this->_filters[] = array ( "type"=>self::SPH_FILTER_STRING_LIST, "attr"=>$attribute, "exclude"=>$exclude, "values"=>$value );
  }
  ...
  public function SetQueryFlag ( $flag_name, $flag_value )
  {
    ...
    if ( $flag_name=="reverse_scan" )	$this->_query_flags = $this->sphSetBit ( $this->_query_flags, 0, $flag_value==1 );
    if ( $flag_name=="sort_method" )	$this->_query_flags = $this->sphSetBit ( $this->_query_flags, 1, $flag_value=="kbuffer" );
    if ( $flag_name=="max_predicted_time" )
    {
      $this->_query_flags = $this->sphSetBit ( $this->_query_flags, 2, $flag_value>0 );
      $this->_predictedtime = (int)$flag_value;
    }
    if ( $flag_name=="boolean_simplify" )	$this->_query_flags = $this->sphSetBit ( $this->_query_flags, 3, $flag_value );
    if ( $flag_name=="idf" && ( $flag_value=="normalized" || $flag_value=="plain" ) )	$this->_query_flags = $this->sphSetBit ( $this->_query_flags, 4, $flag_value=="plain" );
    if ( $flag_name=="global_idf" )	$this->_query_flags = $this->sphSetBit ( $this->_query_flags, 5, $flag_value );
    if ( $flag_name=="idf" && ( $flag_value=="tfidf_normalized" || $flag_value=="tfidf_unnormalized" ) )	$this->_query_flags = $this->sphSetBit ( $this->_query_flags, 6, $flag_value=="tfidf_normalized" );
    if ( $flag_name=="low_priority" ) $this->_query_flags = $this->sphSetBit ( $this->_query_flags, 8, $flag_value );
  }
  ...
  {
    $this->_query_flags = $this->sphSetBit ( 0, 6, true ); // default idf=tfidf_normalized
  ...
  public function AddQuery ( $query, $index="*", $comment="" )
  {
    ...
    foreach ( $this->_filters as $filter )
    {
      ...
      switch ( $filter["type"] )
      {
        ...
        case self::SPH_FILTER_STRING_LIST:
          $req .= pack ( "N", count($filter["values"]) );
          foreach ( $filter["values"] as $value )
    ...
    foreach ( $this->_overrides as $key => $entry )
    {
      ...
      foreach ( $entry["values"] as $id=>$val )
      {
        ...
        switch ( $entry["type"] )
        {
          ...
          case self::SPH_ATTR_BIGINT:	$req .= $this->sphPackI64 ( $val ); break;
      ...
      while ( $count-->0 && $p<$max )
      {
        ...
        foreach ( $attrs as $attr=>$type )
        {
          ...
          } else if ( $type==self::SPH_ATTR_STRING )
          {
            ...
            $p += $val;
          } else if ( $type==self::SPH_ATTR_FACTORS )
          {
