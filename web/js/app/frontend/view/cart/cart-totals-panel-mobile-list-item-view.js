define(function(require) {
  ...
  var templateMobile = _.template('\
  <div class="cards-container__wrap">\
    <div class="cards-container__item">\
      <a href="/app_dev.php/products/skamorca" class="cards-container__item-image">\
        <img src="/images/no_photo.png" alt="">   \
      </a>\
      <div class="cards-container__item-info">\
        <a href="/app_dev.php/products/skamorca" class="item-info__title"><%= name %></a>\
        <span class="item-info__quantity"><%= quantity %> <%= units %> × <%= price.toFixed() %> ₽</span>\
      </div>\
    </div>\
    <div class="cards-container__price"><%= price.toFixed() %></div>\
  </div>');
